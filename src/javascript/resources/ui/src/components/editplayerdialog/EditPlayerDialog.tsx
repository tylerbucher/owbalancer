...
import DeletePlayerDialog from "../deleteplayerdialog/DeletePlayerDialog";
...
type EditPlayerDialogState = {
    ...
    showDeletePlayerDialog: boolean;
};
...
class EditPlayerDialog extends React.Component<EditPlayerDialogProps, EditPlayerDialogState> {
    ...
    render() {
        let ref = this;
        let dialog = <div/>;
        if (this.state.showDeletePlayerDialog) {
            dialog = <DeletePlayerDialog playerName={this.state.username} onClose={() => function () {
                ref.setState({showDeletePlayerDialog: false})
            }} onDelete={function () {
                ref.deleteUserInfo(ref.state.id);
                ref.setState({showDeletePlayerDialog: false});
            }}/>;
        }
            ...
            <form id={"editUserForm"} onSubmit={(e) => this.handleSubmit(e)}>
                ...
                <div className="form-group">
                    ...
                    <Button id={"delButton"} cls={"button alert form-control mb-4"} title={"Delete User"}
                            onClick={(e: any) => {
                                e.preventDefault();
                                this.setState({showDeletePlayerDialog: true});
                            }}/>
                </div>
            </form>
            {dialog}
